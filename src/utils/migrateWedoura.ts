import { createCaseStudy } from '../config/caseStudyService';
import type { CaseStudyFormData } from '../types/caseStudy';

/**
 * Migrates the Wedoura case study to Firebase
 * This creates the project with all content from the existing CaseStudy.tsx
 */
export async function migrateWedouraToFirebase(): Promise<string> {
  const wedouraProject: CaseStudyFormData = {
    // Basic Info
    title: 'Wedoura',
    slug: 'wedoura',
  ...
  try {
    const projectId = await createCaseStudy(wedouraProject);
    console.log('✅ Wedoura case study migrated successfully!');
    console.log('Project ID:', projectId);
    return projectId;
  } catch (error) {
    console.error('❌ Error migrating Wedoura:', error);
    throw error;
  }
}
